...
def cmd_info(args):
    ...
    algorithms = ['AES-128', 'AES-256', 'ChaCha20', 'RSA-2048', 'RSA-4096', 'ECC-256']
    for alg in algorithms:
        print(f"  • {alg}")
    print()
...
def cmd_encrypt(args):
    ...
    try:
        if algorithm == 'AES-128':
            encrypted, metadata = crypto.aes_128_encrypt(data)
        elif algorithm == 'AES-256':
            encrypted, metadata = crypto.aes_256_encrypt(data)
        elif algorithm == 'ChaCha20':
            encrypted, metadata = crypto.chacha20_encrypt(data)
        else:
        ...
        # Save metadata (key, iv/nonce) - in real use, store securely!
        metadata_path = f"{output_path}.meta"
        import pickle
        with open(metadata_path, 'wb') as f:
            pickle.dump({'algorithm': algorithm, **metadata}, f)
        ...
        print()
        print("WARNING: Metadata file contains encryption key. Store securely!")
        print("=" * 50)
...
def cmd_decrypt(args):
    ...
    try:
        if algorithm == 'AES-128':
            decrypted = crypto.aes_128_decrypt(encrypted, metadata['key'], metadata['iv'])
        elif algorithm == 'AES-256':
            decrypted = crypto.aes_256_decrypt(encrypted, metadata['key'], metadata['iv'])
        elif algorithm == 'ChaCha20':
            decrypted = crypto.chacha20_decrypt(encrypted, metadata['key'], metadata['nonce'])
        else:
